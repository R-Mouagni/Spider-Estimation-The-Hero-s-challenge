...
public class QuestionDisplayVB extends VBox {
    //Labels
    ...
    public QuestionDisplayVB(GameSP game, BasicCard card, PlayerView player, String squareClass) {
        this.game = game;
        this.card = card;
        this.player = player;
        ...
        getLblTitle().setText(player.getPlayer().toStringDuel()+"\n"+"Question for theme " + card.getTheme());
    ...
    public Button getBtnSubmitDifficultyChoice() {
        if(btnSubmitDifficultyChoice==null) {
            ...
            btnSubmitDifficultyChoice.setOnAction(e -> {
                ...
                // Event handling for submitting answer
                getBtnSubmitAnswer().setOnAction(h->{
                    ...
                    if(question.checkAnswer(answer)) {
                        ...
                        switch(answer) {
                            case "gadget1234":
                                System.out.println("cheat gadget");
                                getLblVerification().setText("Cheat !");
                                getLblVerification().setTextFill(Color.BLUE);
                                getBtnSubmitAnswer().setDisable(true);
                                game.getGameBoard().movePlayerCheat(player,18);
                                break;
                            case "vilain1234":
                                System.out.println("cheat vilain");
                                player.getPlayer().moveTo(2, 4);
                                getLblVerification().setText("Cheat !");
                                getLblVerification().setTextFill(Color.BLUE);
                                getBtnSubmitAnswer().setDisable(true);
                                game.getGameBoard().movePlayerCheat(player,12);
                    			break;
                            case "boss1234":
                                System.out.println("cheat boss");
                                getLblVerification().setText("Cheat !");
                                getLblVerification().setTextFill(Color.BLUE);
                                getBtnSubmitAnswer().setDisable(true);
                                game.getGameBoard().movePlayerCheat(player,38);                         
                    			break;
                            case "duel1234":
                                System.out.println("cheat duel");
                                getLblVerification().setText("Cheat !");
                                getLblVerification().setTextFill(Color.BLUE);
                                getBtnSubmitAnswer().setDisable(true);
                                game.getGameBoard().movePlayerCheat(player,15);
